...
class ZKLibrary {
		...
		try
		{
			...
			if(count($this->user_data) > 0)
			{
				...
				while(strlen($user_data) > 72)
				{
					...
					$users[$uid] = array("user_id" => $userid, "name" => $name,"role" => intval($role), "password" =>$password);
	...
	public function getAttendance($startdatetime = null, $enddatetime = null)
	{
		if ($this->protocol == 'TCP') {
			$command = 1503;
		...
		try
		{
			...
			if(count($this->attendance_data) > 0)
			{
				...
				while(strlen($attendance_data) > 40)
				{
					...
					$checkStat = hexdec(substr( $u[1], 66, 2 ) );
					$timestamp = $this->decodeTime(hexdec($this->reverseHex(substr($u[1], 58, 8))));
					array_push($attendance, [$uid, "employee_id" => $id, $state, "datetime" => $timestamp, "checkin" => $checkStat]);
					if ($enddatetime != null && $startdatetime != null) {
						$start  = Carbon::createFromFormat('Y-m-d H:i:s', $startdatetime);
						$end  = Carbon::createFromFormat('Y-m-d H:i:s', $enddatetime);
						if (Carbon::createFromFormat('Y-m-d H:i:s', $timestamp)->between($start, $end)) {
							array_push($attendance, [$uid, "employee_id" => $id, $state, "datetime" => $timestamp, "checkin" => $checkStat]);
						}
					} else {
						array_push($attendance, [$uid, "employee_id" => $id, $state, "datetime" => $timestamp, "checkin" => $checkStat]);
